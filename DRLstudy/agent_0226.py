...
class Node:
    ...
    # tile 정보 update(relic node 인지 아닌지) > True/False
    ...
    # tile 정보 update(reward node 인지 아닌지) > True/False
...
class Space:
    ...
    # reward node map update from reward(environment)
    ...
    # 주변에 relic node가 없으면 reward를 받을 수 없으므로 reward=False로 update
    def _update_reward_status_from_relics_distribution(self):
        ...
        # relic node or un-explored node > relic node로 간주
    ...
    def _update_map(self, obs):
        ...
        # visible node에 대해서 type, energy 값 update
        for node in self:
            x, y = node.coordinates
...
# class for agent / node for agent
class Ship:
...
class Fleet:
    ...
    # observation 으로부터 agent 정보 update
...
class Agent:
        ...
        for i, ship in enumerate(ships):
            if ship.action is not None:
                if ship.action == ActionType.sap:
                    actions[i] = ship.action, ship.sap_x, ship.sap_y
                else:
    ...
    def find_relics(self):
        ...
        # 탐색되지 않은 node 모두 target으로 추가 (exploration)
        ...
        def set_task(ship):
            ...
            # target set중에서 가장 가까운 node를 target으로 설정
            target, _ = find_closest_target(ship.coordinates, targets)
            if not target:
                return False

            # A* algorithm을 이용해서 최적 경로 계산
            ...
            if actions and ship.energy >= energy:
                ...
                # 최적 경로 상에 있는 tile의 sensor 범위 안에 있는 node는 target set에서 제거
                for x, y in path:
    ...
    def find_rewards(self):
        # reward node위치 모두 찾았으면, reward 탐색 중단
        ...
        for ship in self.fleet:
            if ship.task == "find_rewards":
                ...
                if (
                    ...
                    # target으로 설정되어있는 relic node set
        ...
        # unexplored relic 중에서 ship의 target으로 설정되지 않은 node를 가장 가까운 ship의 target으로 설정
        for relic in unexplored_relics:
            if relic not in relic_node_to_ship:

                # 가장 가까운 ship의 target node로 설정
        ...
        def set_task(ship, relic_node, can_pause):
            ...
            for x, y in nearby_positions(
                *relic_node.coordinates, Global.RELIC_REWARD_RANGE
                ...
                # explore 되지 않은 relic 주변 node라면 target으로 설정
    ...
    def get_unexplored_relics(self) -> list[Node]:
        # relic node 주변의(시야 범위) node가 모두 explored 된 경우에만 탐색된 relic node로 인정
        relic_nodes = []
    ...
    def harvest(self):
        ...
        def set_task(ship, target_node):
            # ship의 위치가 이미 target node일때, 위치 유지하면서 reward 얻기
            if ship.node == target_node:
                ship.task = "harvest"
                ship.target = target_node
                
                # sap action 시도 하기!!!
                # idea : reward node위에 있는 agent의 시야 안에 상대 agent가 있을 경우에만 공격
                # 공격 조건을 충족하지 못할 경우에는 ActionType.center 유지
                # 상대방 agent들 중에서 가장 가까운 agent를 찾고, 해당 agent가 sap range내에 있을 경우에 공격
                # 공격 좌표는 어디로 해야할까???
                
        
                targets = [opp_ship.node.coordinates for opp_ship in self.opp_fleet if opp_ship.node is not None]
                target, d = find_closest_target(ship.coordinates, targets)
                if target is not None and d <= Global.UNIT_SAP_RANGE and Global.UNIT_SAP_COST < ship.energy :
                    ship.action = ActionType.sap
                    ship.sap_x = target[0]-ship.coordinates[0]
                    ship.sap_y = target[1]-ship.coordinates[1]
        ...
        for ship in self.fleet:
            if ship.task == "harvest":
                ...
                # 임의의 ship의 target으로 설정된 node 저장
        ...
        targets = set()
        # reward node중에서 target으로 설정되지 않은 node를 target후보 set에 등록
        ...
        for ship in self.fleet:
            if ship.task:
                continue
            # task가 harvest로 설정되지 않은 ship의 경우 target 후보 중에 가장 가까운 node를 target으로 설정.
            target, _ = find_closest_target(ship.coordinates, targets)
